...
/*
 * NAME:        query_wiztool()
 * DESCRIPTION: Return the wiztool clone, if any
 */
object
query_wiztool()
{
...
private void tell_audience(string str)
{
	...
	user = users[i];
	if (user != this_object() &&
	    sscanf(object_name(user), SYS_USER + "#%*d") != 0) {
	    user->message(str);
	}
    }
}
...
int login(string str)
{
	...
	restore_object(SYS_USER_DIR + "/" + str + ".pwd");
	...
	if (password) {
	    ...
	    if (!wiztool) {
		wiztool = clone_object(SYS_WIZTOOL, str);
	    }
...
int receive_message(string str)
{
	...
	case STATE_LOGIN:
		...
		wiztool = clone_object(SYS_WIZTOOL, name);
	    }
	    break;
	...
	case STATE_NEWPASSWD2:
		...
		if (wiztool) {
		    /* save wizards only */
		    save_object(SYS_USER_DIR + "/" + name + ".pwd");
		}
		message("\nPassword changed.\n");
	    } else {
