...
/*
 * NAME:        alias_expand()
 * DESCRIPTION: expand any alias at the beginning of a command
 */
static string alias_expand(string command)
{
    string argument;

    if (!sscanf(command, "%s %s", command, argument)) {
        argument = nil;
    }
    if (aliases[command]) {
...
/*
 * NAME:        evt_error()
 * DESCRIPTION: handle an error event from the creature
 */
static void evt_error(object sender, string message)
{
